"""
The training/validating script for ViP on ImageNet classification.
This file is based on the training script of timm library (version 0.3.4)
developed by Ross Wightman (https://github.com/rwightman/pytorch-image-models/)
"""
import os
import time
import logging
...
from utils.init import args, args_text, model_params
...
def main():
    ...
    if args.resume:
        ...
        out_dir = os.path.join(output_base, "train", exp_name)
        if args.local_rank == 0 and not os.path.exists(out_dir):
            os.makedirs(out_dir)
        else:
            resume = args.resume
    ...
    if args.distributed:
        ...
        if has_apex and use_amp != 'native':
            ...
            model = NativeDDP(model, device_ids=list(range(args.num_gpu)))  # can use device str in Torch >= 1.1
